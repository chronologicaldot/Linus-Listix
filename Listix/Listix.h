...
namespace util
{
	...
	{
	public:
		typedef  Unk          value_type;
		typedef  Listix<Unk>  self_type;
#ifdef _MSC_VER
		typedef  __int32      index_type;
#else
		...
		{
			index_type failIndex;
		public:
			NoItemException(index_type i=0)
				: failIndex(i)
			{}

			index_type getFailureIndex() const {
				return failIndex;
			}
		};

	private:
		index_type size; // list size
		...
		Node* Selector;
		index_type selectorIndex; // location of Selector
	...
	public:
		...
		index_type getSelectorIndex() const {
		...
		Unk& getCurrentItem() {
			...
			//! Check bounds (simple)
		bool isInBounds( index_type index ) const {
		...
		*/
		bool isOutOfBounds( index_type* index, bool canAlterIndex=false) const
		{
		...
		Throws NoItemException if there are no items in the list. */
		Unk* getItemPtrAt( index_type index )
		{
			if ( size == 0 )
				throw NoItemException(index);
			return &((this->at(index))->Item);
		...
		bool isIndexTheStart( index_type index ) const
		{
			return (index == 0);
		}
		...
		bool isIndexTheEnd( index_type index ) const
		{
		...
		void replace( index_type index, Unk item )
		{
		...
		*/
		...
		void insert( Unk item, index_type index )
		{
		...
		void deleteAt( index_type index )
		{
			if ( !(size > 0) || isOutOfBounds( &index, true ) )
				return;
		...
		void swap( index_type item1_index, index_type item2_index )
		{
		...
		bool relocate( index_type index, index_type newIndex )
		{
			// Account for indexes that are out of bounds
		...
		*/
		...
		{
			for ( index_type i = 0; i < list.getSize(); i++ )
			{
		...
		void appendRange( Listix<Unk>& fromList, index_type startIndex, index_type endIndex)
		{
			for ( index_type i = startIndex; i < endIndex; i++ ) {
				push_back( fromList.copyOf(i) );
			}
		}

			//! Destroy list front
		void destroyFront( index_type newStartIndex )
		{
			if ( newStartIndex < 0 )
				return;
			...
			//! Destroy list back
		void destroyBack( index_type newEndIndex ) {
			if ( newEndIndex >= size )
				return;
			if ( newEndIndex < 0 ) {
				clear();
		...
		void destroyRange( index_type start, index_type end ) {
			if ( size == 0 )
				return;
